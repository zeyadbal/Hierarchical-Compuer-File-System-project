...
string CpCommand::toString() { string st; st+="cp "; st+=this->getArgs(); return st;}
...
void CpCommand::execute(FileSystem &fs){
	...
	cdCMD1.execute(fs);
	if(!cdCMD1.getFoundVal()){	cout<<"No such file or directory"<<endl;fs.setWorkingDirectory(currDir);verbose=vrbs;	return; }
	else{
	...
	if(!cdCMD2.getFoundVal()){	cout<<"No such file or directory"<<endl;fs.setWorkingDirectory(currDir);verbose=vrbs;	return; }
	else{
		bool exist=false;
...
void CpCommand::addAllChildsToFsAllDirectoriesRecursively(FileSystem &fs, Directory *newDir){
